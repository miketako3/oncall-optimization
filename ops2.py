...
def solve_oncall_scheduling():
    ...
    # 週末・祝日最大値変数
    max_weekend_holiday = solver.NumVar(0, solver.infinity(), 'max_weekend_holiday')
    # 週末・祝日最小値変数
    min_weekend_holiday = solver.NumVar(0, solver.infinity(), 'min_weekend_holiday')
    # 各エンジニアの週末・祝日シフト数を計算する変数
    weekend_holiday_shifts_var = {}
    for i in range(NUM_ENGINEERS):
        weekend_holiday_shifts_var[i] = solver.NumVar(0, solver.infinity(), f'weekend_holiday_shifts[{i}]')
    ...
    for i in range(NUM_ENGINEERS):
        # 各エンジニアの週末・祝日シフト数を計算
        solver.Add(
            weekend_holiday_shifts_var[i] == sum((x[i, k] + 0.5 * backup[i, k]) * (is_weekend[k] or is_holiday[k]) 
                                                for k in range(NUM_DAYS)),
            f'weekend_holiday_shifts_def[{i}]'
        )
        
        # 最大値制約
        solver.Add(
            max_weekend_holiday >= weekend_holiday_shifts_var[i],
            f'max_weekend_holiday_constraint[{i}]'
        )
        
        # 最小値制約
        solver.Add(
            min_weekend_holiday <= weekend_holiday_shifts_var[i],
            f'min_weekend_holiday_constraint[{i}]'
        )

    # minimax目標: 最大値と最小値の差を最小化
    minimax_weekend_diff = solver.NumVar(0, solver.infinity(), 'minimax_weekend_diff')
    solver.Add(
        minimax_weekend_diff >= max_weekend_holiday - min_weekend_holiday,
        'minimax_weekend_diff_def'
    )
    ...
    for i in range(NUM_ENGINEERS):
        objective.SetCoefficient(minimax_weekend_diff, lambda4 * 5)
    ...
    if status == pywraplp.Solver.OPTIMAL or status == pywraplp.Solver.FEASIBLE:
        ...
        total_weekend_diff = minimax_weekend_diff.solution_value()
        print(f"週末・祝日担当バランス最大最小差: {total_weekend_diff}")
        print(f"週末・祝日担当バランス違反ペナルティ: {lambda4 * 5 * total_weekend_diff}")
        ...
        print("\n各エンジニアの週末・祝日シフト数:")
        for i in range(NUM_ENGINEERS):
            engineer_weekend_shifts = sum((schedule[i, k] + 0.5 * backup_schedule[i, k]) * (is_weekend[k] or is_holiday[k]) 
                                        for k in range(NUM_DAYS))
            print(f"エンジニア {i}: {engineer_weekend_shifts}")
